(function () {
    ...
    var pForm = document.querySelector('form[data-ajax-admin-profile]');
    if (pForm) {
      pForm.addEventListener('submit', function (e) {
        ...
        // vacíos para que el servidor nunca exija completarlos.
        var pwCheck = document.getElementById('changePasswordCheck');
    ...
    var phForm = document.querySelector('form[data-ajax-admin-photo]');
    if (phForm) {
    ...
    var deact = document.querySelector('form[data-ajax-admin-deactivate]');
    if (deact) {
      deact.addEventListener('submit', function (e) {
        e.preventDefault();
        window.App && App.confirmModal(
          '¿Seguro que deseas desactivar tu cuenta? No podrás acceder hasta que se reactive.',
          'Desactivar cuenta'
        ).then(function (ok) {
          if (!ok) return;
